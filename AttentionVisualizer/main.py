...
class AttentionVisualizer:
    ...
    def __init__(self, model_name: str = "roberta-base"):
        ...
        self.ignore_specials = widgets.Checkbox(
            value=True, description="Ignore BOS/EOS", indent=False
        )
        self.ignore_dots = widgets.Checkbox(
            value=True, description="Ignore [dot]s", indent=False
        )
        self.ignore_stopwords = widgets.Checkbox(
            value=True, description="Ignore Stop Words", indent=False
        )
        self.options = widgets.HBox(
            [self.ignore_specials, self.ignore_dots, self.ignore_stopwords]
        )
        ...
        self.layer = widgets.Dropdown(
            options=["all", "range", "individual"], value="all", description="Layer"
        )
        self.layer_selection = widgets.HBox(
            [self.layer, self.layer_range, self.layer_ind]
        )

        self.head_range = widgets.IntRangeSlider(
            value=[3, 5],
            min=1,
            max=12,
            step=1,
        ...
        self.head = widgets.Dropdown(
            options=["all", "range", "individual"], value="all", description="Head"
        )
        ...
        self.note_lbl = widgets.HTML(
            value=f"<small>Hold your cursor on each word for a second to see its attention score.</small>"
        )

        self.out = widgets.HTML(
            layout={"border": "1px solid black", "padding": "4px", "margin-top": "10px"}
        )

        self.step2 = widgets.VBox(
            [
                self.preview_config_lbl,
                self.options,
    ...
    def on_visualize_click(self, c):
        ...
        inputs = self.tokenizer(
            self.input_text.value, return_tensors="pt", max_length=512, truncation=True
        )
        outputs = self.model(**inputs, output_attentions=True)

        the_tokens = self.tokenizer.convert_ids_to_tokens(
            inputs.input_ids[0], skip_special_tokens=self.ignore_specials.value
        )
        number_of_tokens = inputs.input_ids.size(1)
        positions, dot_positions, stopwords_positions = find_positions(
            self.ignore_specials.value,
            self.ignore_stopwords.value,
            the_tokens,
            self.stop_words,
        )
        the_words = make_the_words(
            self.input_text.value, positions, self.ignore_specials.value
        )
        ...
        for i in range(*layer_indexes):
            for ii in range(*head_indexes):
                the_scores.append(
                    torch.sum(outputs.attentions[i][0][ii], dim=0) / number_of_tokens
                )
